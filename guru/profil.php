...
$my_stats = ['articles' => 0, 'published' => 0, 'likes' => 0, 'comments' => 0];

$stmt = mysqli_prepare($conn, "SELECT COUNT(*) as total FROM artikel WHERE author_id = ?");
if ($stmt) {
    mysqli_stmt_bind_param($stmt, "i", $user_id);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
...
$stmt = mysqli_prepare($conn, "SELECT COUNT(*) as total FROM artikel WHERE author_id = ? AND status = 'published'");
if ($stmt) {
    mysqli_stmt_bind_param($stmt, "i", $user_id);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
    $my_stats['published'] = mysqli_fetch_assoc($result)['total'];
    mysqli_stmt_close($stmt);
}

$stmt = mysqli_prepare($conn, "SELECT COUNT(*) as total FROM likes WHERE user_id = ?");
if ($stmt) {
    mysqli_stmt_bind_param($stmt, "i", $user_id);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
    $my_stats['likes'] = mysqli_fetch_assoc($result)['total'];
    mysqli_stmt_close($stmt);
}
    ...
    <div class="main-content">
        ...
        <div class="profile-card">
            <div class="profile-header">
                ...
                <h2><?php echo htmlspecialchars($guru_data['nama'] ?? 'Nama tidak tersedia', ENT_QUOTES, 'UTF-8'); ?></h2>
                <p style="color: #666;">Guru - <?php echo htmlspecialchars($guru_data['username'] ?? 'Username tidak tersedia', ENT_QUOTES, 'UTF-8'); ?></p>
            </div>
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    <div class="stat-number"><?php echo (int)$my_stats['published']; ?></div>
                    <div class="stat-label">Published</div>
                </div>
                <div class="stat-card">
                    <div class="stat-number"><?php echo (int)$my_stats['likes']; ?></div>
            ...
            <div class="achievement-section">
                ...
                <div class="achievement-badges">
                    <?php if ($my_stats['articles'] > 0): ?>
                        <span class="badge"><i class="fas fa-pen"></i> Penulis</span>
                    <?php endif; ?>
                    <?php if ($my_stats['published'] >= 5): ?>
                        <span class="badge"><i class="fas fa-star"></i> Guru Aktif</span>
                    <?php endif; ?>
                    <?php if ($my_stats['likes'] >= 20): ?>
                        <span class="badge"><i class="fas fa-heart"></i> Populer</span>
                    <?php endif; ?>
                    <?php if ($my_stats['comments'] >= 10): ?>
                        <span class="badge"><i class="fas fa-comments"></i> Aktif Berkomentar</span>
                    <?php endif; ?>
                    <?php if ($my_stats['published'] >= 10): ?>
                        <span class="badge"><i class="fas fa-crown"></i> Guru Berprestasi</span>
                    <?php endif; ?>
                    <?php if ($my_stats['articles'] == 0 && $my_stats['likes'] == 0 && $my_stats['comments'] == 0): ?>
                        <span class="badge"><i class="fas fa-seedling"></i> Pemula</span>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <form method="POST">
                <div class="form-group">
                    ...
                    <input type="text" id="nama" name="nama" class="form-control" 
                           value="<?php echo htmlspecialchars($guru_data['nama'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" 
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="username" name="username" class="form-control" 
                           value="<?php echo htmlspecialchars($guru_data['username'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" 
